...
from corr_structures import LB2013CorrelationModel
from openquake.hazardlib.correlation import JB2009CorrelationModel
from openquake.hazardlib.correlation import HM2018CorrelationModel
...
def main(var, r, voi, rand, cor_model, vs_corr):
    ...
    elif cor_model == 'LB2013':
        CM = LB2013CorrelationModel()
    else:
        CM = HM2018CorrelationModel()
    ...
    if len(var['uncertaintydata'])>2:
        ...
        X = np.multiply(COR, var['uncertaintydata'][0]) 
        if voi == 'MMI':
            DATA_NEW = DATA + X + 2*var['uncertaintydata'][1]
        else:    
            DATA_NEW = np.multiply(DATA,np.exp(X + 1.5*var['uncertaintydata'][1]))
...
def calculate_corr(dist_mat, voi, CM):
    ...
    """
    if voi == 'MMI':
        correlation_model = CM._get_correlation_matrix(dist_mat, from_string('SA(1.0)'))
    else:
        correlation_model = CM._get_correlation_matrix(dist_mat, from_string(voi))
...
def inc_stations(j,i,N,K,r,site_collection_SM, site_collection_station, dist_mat, X, inc_ind):
    ...
    """
    if str((type(site_collection_station))) != "<class 'openquake.hazardlib.site.SiteCollection'>":
        inc_sta_indices = np.array([])
    else:    
